...
import SinglePostAdvertise from "../models/SinglePostAdvertise.model.js";
...
export const singlepostaddAdvertise = async (req,res,next)=>{
    try {
        ...
        const adverise = new SinglePostAdvertise({
            image: advertiseImage,
...
export const singlepostshowAdvertise = async (req,res,next)=>{
    try {
        

        const advertise = await SinglePostAdvertise.find().sort({createdAt : -1}).lean().exec()
  ...
  export const singleposteditAdvertise = async (req,res,next)=>{
    try {
        const {advertiseid} = req.params
        
        
        const advertise = await SinglePostAdvertise.findById(advertiseid)
        if (!advertise) {
            next(handleError(404,'Data not found.'))
        }
  ...
  export const singlepostupdateAdvertise = async (req,res,next)=>{
    try {
        const {advertiseid} = req.params
        ...
        const adverise = await SinglePostAdvertise.findById(advertiseid)
        ...
        const newAdvertise = await SinglePostAdvertise.findByIdAndUpdate(advertiseid,{
...
export const singlepostdeleteAdvertise = async (req,res,next)=>{
    try {
        const {advertiseid} = req.params
        await SinglePostAdvertise.findByIdAndDelete(advertiseid)
        res.status(200).json({
            success : true,
            message: 'Advertise Deleted Successfully.'
        })
